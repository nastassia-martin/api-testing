...
describe('OrdersAPI', () => {
    ...
    it('should create and then return that order', async () => {
        ...
        expect((order.customer_first_name).length).toBeGreaterThanOrEqual(2)
        expect((order.customer_last_name).length).toBeGreaterThanOrEqual(2)
        expect((order.customer_address).length).toBeGreaterThanOrEqual(5)
        expect((order.customer_postcode).length).toBeGreaterThanOrEqual(5)
        expect((order.customer_email)).toMatch(/^\S+@\S+\.\S+$/)
        expect((order.order_total)).toBeGreaterThanOrEqual(1)
        expect((order.order_items)?.length).toBeGreaterThanOrEqual(1)
        expect(order.order_items[0].item_price).toBeGreaterThanOrEqual(1)
        expect(order.order_items[0].item_total).toBeGreaterThanOrEqual(1)
        expect(order.order_items[0].qty).toBeGreaterThanOrEqual(1)
        expect(order.order_items[0].product_id).toBeDefined()
    })
})